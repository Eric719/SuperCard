...
- (void)drawPips
{
    ...
    if (self.rank == 6 || self.rank == 7 || self.rank == 0) {
        [self drawPipsWithHorizontalOffset:PIP_HOFFSET_PERCENTAGE
                            verticalOffset:0
                        mirroredVertically:NO];
    }
    if (self.rank == 2 || self.rank == 3 || self.rank == 7 || self.rank == 0 || self.rank == 10) {
        [self drawPipsWithHorizontalOffset:0
...
- (void)drawCorners
{
    ...
    //将它旋转上下颠倒
    CGContextRef context = UIGraphicsGetCurrentContext();//得到上下文的方法
    //ＴranslateCTM是变换矩阵
    CGContextTranslateCTM(context, self.bounds.size.width, self.bounds.size.height);//放到右下角
    CGContextRotateCTM(context, M_PI);//M_PI 就是180度 pai弧度的半圈是180度
...
- (void)setup
{
    ...
    self.opaque = NO;
    self.contentMode = UIViewContentModeRedraw;//如果self.bounds变化了就要重绘
}
